class BSTNode:
    ...
    def put(self, key):
        ...
        # adding a new node to the tree with the specified key
        if key < self.key:
            if self.left is None:
                # if the left child is None create a new node and set it as the left child
                self.left = BSTNode(key)
            else:
                # otherwise recursively call put on the left child
                self.left.put(key)

        # adding new node to the right if key is greater than self.key
        elif key > self.key:
    ...
    def pre_order(self):
        # defines a generator for pre-order traversal
        yield self.key
    ...
    def post_order(self):
        # define a generator for post-order traversal
        if self.left is not None:
            # recursively traverses the left subtree
        ...
        # yield the key of the current node
        yield self.key
