class LLNode:
    ...
    def _repr(self, item_list):
        ...
        if self.link is None: item_list.append(f"{self.data}")     # Base case - add final item
        else:
    ...
    # Use recursion.
    # Non-recursive solutions will not recieve credit.
    def add_last(self, item):
        ...
        if self.link is None:
            # We are the last node so we create a new node
    ...
    def reverse(self, prev=None):
        ...
        tail node updates it’s link (to whatever was passed in as prev), 
        then returns itself every other node updates its link, then return the old tail
        """

        # If we are at the tail
        if self.link is None:
            # update the link to the prev
            self.link = prev
            return self
        else:
            # Returns the current node as the new head
